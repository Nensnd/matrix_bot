...
async def view(message: types.Message):
    ...
    new_im.save(f'{message.from_user.id}view.png', "PNG")
    new_im.close()
    new_im = Image.open(f'{message.from_user.id}view.png')
    cx = cx - 1
    cy = cy - 1
    croped = new_im.crop(((int(x) - cx * 2048) - 512, (int(y) - cy * 2048) - 512, (int(x) - cx * 2048) + 513,(int(y) - cy * 2048) + 513))
    croped.save(f'{message.from_user.id}view.png', 'PNG')

    await message.reply_document(file(f'{message.from_user.id}view.png'))
    os.remove(f'{message.from_user.id}view.png')

# Хэндлер на команду /add
@dp.message(Command("add"))
async def reply_builder(message: types.Message):
    if message.text.replace('/add', '').replace('/Add', '').replace('/add ', '').replace('/Add', '') == '':
    ...
    elif len(message.text.replace('/add ', '').replace('/Add ', '').split()) == 3:
        color = message.text.replace('/add ', '').replace('/Add ', '').split(' ')[0]
        global work
        work = 1
        x = int(message.text.replace('/add ', '').replace('/Add ', '').split(' ')[1])
        y = 10239 - int(message.text.replace('/add ', '').replace('/Add ', '').split(' ')[2])
        ...
        new_im.paste(img_03, (0, img_01_size[1]))
        new_im.paste(img_04, (img_01_size[0], img_01_size[1]))
        new_im.save(f'{message.from_user.id}merged_matrix.png', "PNG")

        new_im.close()
        new_im = Image.open(f'{message.from_user.id}merged_matrix.png')
        cx = cx - 1
        cy = cy - 1
        ...
        croped = new_im.crop(((int(x) - cx * 2048) - 128, (int(y) - cy * 2048) - 128, (int(x) - cx * 2048) + 129,
                              (int(y) - cy * 2048) + 129))
        '''c_end = croped.resize((2561, 2561))'''
        croped.save(f'{message.from_user.id}croped.png', 'PNG')
        await message.reply("Ваш запрос обработан!\nМатрица изменена!")
        await message.reply_document(file(f'{message.from_user.id}croped.png'))
        os.remove(f'{message.from_user.id}croped.png')
        work = 0
        ...
        # img_02
            ...
            draw.rectangle(
                (int(x) - 2048 - cx * 2048, int(y) - cy * 2048, int(x) - 2048 - cx * 2048, int(y) - cy * 2048),
                fill=color)
        ...
        # img_03
            ...
            draw = ImageDraw.Draw(img)
            draw.rectangle(
                (int(x) - cx * 2048, int(y) - 2048 - cy * 2048, int(x) - cx * 2048, int(y) - 2048 - cy * 2048),
                fill=color)
        ...
        # img_04
            ...
            img = img_04
            draw = ImageDraw.Draw(img)
            draw.rectangle((int(x) - 2048 - cx * 2048, int(y) - 2048 - cy * 2048, int(x) - 2048 - cx * 2048,
                            int(y) - 2048 - cy * 2048), fill=color)
        ...
        os.remove(f'{message.from_user.id}merged_matrix.png')
    else:
        await message.answer("Неправильно составлен запрос!")
